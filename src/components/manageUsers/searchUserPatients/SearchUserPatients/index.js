...
const resetSerachInput = () => {
    document.getElementById('searchUserPatientsForm').reset()
...
export default function SearchUpdateUsers({ setUsers, alertMessage, setAlertMessage }) {
    ...
    const searchUsers = async (event) => {
        ...
        const response = await Fetch.Search('admin/users/patients/search', searchQueryParam, localStorage.Token)
        if (response) {
    ...
    return (
        <div className="search-user-patients">
            ...
            />
            <form id="searchUserPatientsForm">
